...
app.get('/user', (req, res) => {
  ...
  db.query('SELECT id, username, balance, isAdmin FROM users WHERE id = ?', [req.session.userId], (err, results) => {
    if (err || results.length === 0) return res.status(500).send({ error: 'User not found' });
    res.send(results[0]);
  });
});

// Fetch current open poll and responses
app.get('/poll', (req, res) => {
  db.query('SELECT * FROM polls WHERE status = "open" ORDER BY id DESC LIMIT 1', (err, polls) => {
    if (err) return res.status(500).send(err);
    const poll = polls[0];
    if (!poll) return res.status(404).send({ error: "No active poll found" });
...
app.post('/admin/poll', (req, res) => {
  if (!req.session.isAdmin) return res.status(403).send({ error: 'Forbidden' });
  const { title, dateTime, note } = req.body;
  db.query(
    'INSERT INTO polls (title, date_time, note, status) VALUES (?, ?, ?, "open")',
    [title, dateTime, note],
    (err, result) => {
      if (err) return res.status(500).send({ error: "Error adding new poll" });
